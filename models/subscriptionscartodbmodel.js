...
SubscriptionsCartoDBModel.prototype.createTable = function (sub, cb) {
  ...
  var sql = ["SELECT count(*) as n FROM urbo_get_user_tables('" + this._user + "')", " WHERE urbo_get_user_tables = '{{table}}'"];
  ...
  this.query({
    ...
    if (!data.rows[0].n) {
      ...
      for (var i in subAttr) {
        ...
        if (attr.cartodb) {
          var name;
          if (attr.type === 'coords' || attr.type.startsWith('geojson')) name = 'the_geom';else if ("namedb" in attr) name = attr.namedb;else name = attr.name;
          if (attr.indexdb && name != 'position') {
            if (attr.type == 'json') jsonAttr.push(name);else indexAttr.push(name);
          }
      ...
      var cartodbfy = that._enterprise ? "SELECT CDB_Cartodbfytable('" + that._user + "','" + schemaTable + "');" : "SELECT CDB_Cartodbfytable('" + schemaTable + "');";

      var q = ['CREATE TABLE ' + tableName + ' (', fields.join(','), ");", cartodbfy, 'ALTER TABLE ' + tableName + " ADD COLUMN created_at timestamp without time zone DEFAULT (now() at time zone 'utc');", 'ALTER TABLE ' + tableName + " ADD COLUMN updated_at timestamp without time zone DEFAULT (now() at time zone 'utc');"];

      if (sub.mode == 'update') q.push('ALTER TABLE ' + tableName + ' ADD CONSTRAINT ' + schemaTable + '_id_entity UNIQUE (id_entity);');
      ...
      that.query({ 'query': q.join(' ') }, function (err, data) {
        if (err) {
          ...
          if (jsonAttr.length > 0) that.createJSONIndexes(sub, jsonAttr);

          if (indexAttr.length > 0) that.createAttrIndexes(sub, indexAttr);
      ...
      that.query({
        ...
        var attributes = _.filter(subAttr, function (attr) {
          return attr.cartodb && attr.type != 'coords' && !attr.type.startsWith('geojson');
        });
        var needed = _.map(attributes, function (at) {
          return at.namedb || at.name;
        }).concat('cartodb_id', 'the_geom', 'the_geom_webmercator');
...
SubscriptionsCartoDBModel.prototype.createJSONIndexes = function (sub, attribs) {
  ...
  for (var i = 0; i < attribs.length; i++) {
    q = ['CREATE INDEX ' + sub.schemaname + '_' + sub.id + '_' + attribs[i] + '_idx', 'ON', sub.schemaname + '_' + sub.id, 'USING gin(' + utils.wrapStrings(attribs[i], ['"']) + ')'];
...
SubscriptionsCartoDBModel.prototype.createAttrIndexes = function (sub, attribs) {
  ...
  var sq;
  for (var i = 0; i < attribs.length; i++) {
    sq = ['CREATE INDEX ' + sub.schemaname + '_' + sub.id + '_' + attribs[i] + '_idx', 'ON', sub.schemaname + '_' + sub.id, 'USING btree(' + utils.wrapStrings(attribs[i], ['"']) + ');'];
    q.push(sq.join(' '));
  }
  ...
  */
  ...
  function buildQueryArrays(obj, quote) {
    for (var o in obj) {
      ...
      if (quote) v = "'" + v + "'";
      insert.push(v);
      if (o != 'id_entity') update.push('"' + o + '"=' + v);
    }
  }
  ...
  this.query({ 'query': sql }, function (err, r) {
    if (err) log.error('CARTO: Cannot execute upsert query: %s', sql);
    if (cb) cb(err);
  });
};
...
SubscriptionsCartoDBModel.prototype.storeData = function (sub, contextResponses, cb) {
  ...
  for (var i in contextResponses) {
    ...
    var valid_attrs = _.pluck(_.filter(subAttr, function (attr) {
      return attr.cartodb || attr.type == 'coords' || attr.type.startsWith('geojson');
    }), 'name');
    ...
    if ("mode" in sub && sub.mode == "update") this.upsertSubscriptedData(schemaTable, obj, objdq, cb);else this.insert(schemaTable, obj, objdq, cb);
  }
};
