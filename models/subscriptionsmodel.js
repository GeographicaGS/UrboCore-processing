...
SubscriptionsModel.prototype.createSchema = function (schema, cb) {
  var sql = ['SELECT * FROM information_schema.schemata', 'WHERE schema_name = $1'];
  ...
  this.query(sql.join(' '), [schemaName, sub.id], function (err, data) {
    ...
    if (!data.rows.length) {
      ...
      for (var i in subAttr) {
        ...
        if (attrName == 'position') geomIndex = true;else if (attr.indexdb) {
          if (attr.type == 'json') jsonAttr.push(attrName);else indexAttr.push(attrName);
        }
        fields.push(utils.wrapStrings(attrName, ['"']) + ' ' + utils.getPostgresType(attr.type));
      }
      var q = ['CREATE TABLE', schemaName + '.' + sub.id, '( id bigserial CONSTRAINT', sub.id + '2_pk PRIMARY KEY,', fields.join(','), ",id_entity varchar(64) not null", ",created_at timestamp without time zone DEFAULT (now() at time zone 'utc')", ",updated_at timestamp without time zone DEFAULT (now() at time zone 'utc')"];

      if (sub.mode == 'update') q.push(', CONSTRAINT ' + schemaName + '_' + sub.id + '_id_entity UNIQUE (id_entity)');
      ...
      that.query(q.join(' '), null, function (err, data) {
        ...
        if (geomIndex) that.createGeomIndexes(sub);
      ...
      sql = ['SELECT column_name, data_type, character_maximum_length', 'FROM information_schema.columns', 'WHERE table_schema = $1 AND table_name = $2'];
      ...
      that.query(sql.join(' '), [schemaName, sub.id], function (err, data) {
        ...
        var needed = _.map(subAttr, function (at) {
          return at.namedb || at.name;
        }).concat('id', 'created_at', 'updated_at', 'id_entity');
...
SubscriptionsModel.prototype.createGeomIndexes = function (sub) {
  var q = ['CREATE INDEX ' + sub.schemaname + '_' + sub.id + '_gidx', 'ON', sub.schemaname + '.' + sub.id, 'USING gist(position)'];
...
SubscriptionsModel.prototype.createJSONIndexes = function (sub, attribs) {
  ...
  for (var i = 0; i < attribs.length; i++) {
    q = ['CREATE INDEX ' + sub.schemaname + '_' + sub.id + '_' + attribs[i] + '_idx', 'ON', sub.schemaname + '.' + sub.id, 'USING gin(' + utils.wrapStrings(attribs[i], ['"']) + ')'];
...
SubscriptionsModel.prototype.createAttrIndexes = function (sub, attribs) {
  var q;
  for (var i = 0; i < attribs.length; i++) {
    q = ['CREATE INDEX ' + sub.schemaname + '_' + sub.id + '_' + attribs[i] + '_idx', 'ON', sub.schemaname + '.' + sub.id, 'USING btree(' + utils.wrapStrings(attribs[i], ['"']) + ')'];
...
SubscriptionsModel.prototype.handleSubscriptionsTable = function (data, cb) {
  ...
  this.query(sql, [data.id_name, data.schema], function (err, r) {
    if (err) return log.error('Cannot execute sql query');
...
SubscriptionsModel.prototype.upsertSubscriptedData = function (table, obj, objdq, cb) {
  ...
  this.query(sql, null, function (err, r) {
    if (err) log.error('Cannot execute upsert query: %s' + sql);
